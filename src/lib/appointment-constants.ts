...
export const BOOKING_RULES = {
  ...
  workDays: [1, 2, 3, 4, 5, 6], // 一~六（房仲週六是主戰場，週日先關；要改回原設定就刪掉 6）
...
export const INTENTS = [
  { key: "buy", label: "買房", emoji: "🏠", placeholder: "例:想找頭份後庄三房、預算 1,200 萬、自住、明年要交屋" },
  { key: "sell", label: "賣房", emoji: "🏷️", placeholder: "例:竹南大埔一間三房想賣、想先知道行情跟要繳多少稅" },
...
export const MEET_TYPES = [
  { key: "office", label: "門市面談", emoji: "🏢", desc: "有巢氏房屋 頭份中央帝景加盟店・苗栗縣頭份市公園三街119號" },
  { key: "onsite", label: "現場看屋", emoji: "🏠", desc: "竹南・頭份，約在物件現場" },
  { key: "phone", label: "電話聯繫", emoji: "📞", desc: "建達主動來電" },
  { key: "video", label: "線上視訊", emoji: "💻", desc: "Google Meet / LINE 視訊" },
  // 2026-06-25：客戶自己指定見面地點（Google Places 自動完成 + 純文字備案），要後台核准
  { key: "custom", label: "我指定地點", emoji: "📍", desc: "你來指定見面地點（我確認後才成立）" },
] as const;
...
export const APPOINTMENT_MEETING_POLICIES: Record<MeetTypeKey, AppointmentMeetingPolicy> = {
  ...
  // 看屋要移動，前後各留 30 分；至少提前一天，才有時間跟屋主/管理室橋時間
  onsite: { leadHours: 24, bufferBeforeMin: 30, bufferAfterMin: 30, publicDurations: [60, 90] },
