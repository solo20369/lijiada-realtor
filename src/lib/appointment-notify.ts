 ...
 * 新預約成立 → ① email 通知系統擁有者 ② LINE 通知系統擁有者（建達 admin 群）③ email 確認給客戶。
 *
 * 🔴 寄信安全閘（對齊寄信紅線）：
...
export function buildAppointmentIcs(opts: {
  ...
  return [
    ...
    `SUMMARY:${icsEsc(`與建達預約：${opts.name}`)}`,
...
/** 竹南頭份房仲 CIS 亮色 email 外殼（天藍 header + 橘 CTA + 深字,RWD 手機不跑版,內聯 style）*/
...
<body style="margin:0;padding:0;background:#f7faf8;font-family:'Noto Sans TC','PingFang TC','Microsoft JhengHei',sans-serif;color:#173d30;">
<div style="display:none;max-height:0;overflow:hidden;opacity:0">${esc(opts.preheader || "")}</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f7faf8;padding:24px 12px">
<tr><td align="center">
<table role="presentation" class="container" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:#FFFFFF;border-radius:16px;overflow:hidden;box-shadow:0 4px 20px rgba(28,45,58,0.08)">
  <tr><td style="background:linear-gradient(135deg,#4faf38,#3e8e2c);padding:22px 32px">
    <div style="font-size:13px;color:#f2f7f3;letter-spacing:1px">李建達 ‧ 竹南頭份房仲</div>
...
<body style="margin:0;padding:0;background:#F4F6F8;font-family:'Noto Sans TC','PingFang TC','Microsoft JhengHei',sans-serif;color:#173d30;">
<div style="display:none;max-height:0;overflow:hidden;opacity:0">${esc(opts.preheader || "")}</div>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F4F6F8;padding:24px 12px">
<tr><td align="center">
<table role="presentation" class="container" width="560" cellpadding="0" cellspacing="0" style="width:560px;max-width:560px;background:#FFFFFF;border:1px solid #d8e7dd;border-radius:12px;overflow:hidden">
  <tr><td style="height:4px;background:#4faf38;font-size:0;line-height:0">&nbsp;</td></tr>
  <tr><td class="px" style="padding:22px 30px 4px">
    <div style="font-size:13px;color:#64776f">竹南頭份房仲 · 李建達</div>
    <div style="font-size:20px;font-weight:700;color:#173d30;margin-top:4px">${esc(opts.title)}</div>
  </td></tr>
...
export async function sendCustomerAppointmentConfirmation(a: NotifyInput): Promise<boolean> {
  ...
  const custBody = `
    <p style="font-size:16px;line-height:1.7;margin:0 0 16px">${esc(a.name)} ${honor}你好,建達已經收到你的預約,時段幫你安排好了。</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:15px;line-height:1.9;background:#f7faf8;border-radius:10px">
      <tr><td style="padding:14px 16px">
        <div>📅 時段　<b>${esc(slotTw)}</b></div>
        <div style="margin-top:4px">📍 方式　<b>${esc(meetStr)}</b>${loc ? `<br><span style="color:#64776f;font-size:14px">　${esc(loc.name)}${loc.address ? ` · ${esc(loc.address)}` : ""}</span>` : ""}</div>
        ${a.meetUrl ? `<div style="margin-top:4px">🎥 視訊　<a href="${esc(a.meetUrl)}" style="color:#005335">點我加入 Google Meet</a></div>` : ""}
      </td></tr>
    </table>
    <div style="text-align:center;margin:22px 0 8px">
      <a href="${esc(icsUrl)}" style="display:inline-block;background:#173d30;color:#ffffff;font-size:15px;font-weight:700;padding:12px 28px;border-radius:8px;text-decoration:none">加入行事曆</a>
    </div>
    <p style="text-align:center;font-size:13px;color:#64776f;line-height:2;margin:6px 0 0">
      <a href="${esc(manageUrl)}" style="color:#005335;text-decoration:none">改期 / 取消</a>${mapUrl ? ` · <a href="${esc(mapUrl)}" style="color:#005335;text-decoration:none">地圖導航</a>` : ""} · <a href="${esc(ABIN_PRIVATE_LINE_URL)}" style="color:#005335;text-decoration:none">加建達 LINE</a>
    </p>
    <p style="font-size:14px;color:#64776f;line-height:1.7;margin:18px 0 0">有任何狀況,直接回覆這封信或加建達 LINE 都可以。</p>
  `;
  return sendAppointmentEmailAndRecord({
    appointmentId: a.id,
    purpose: "customer_confirmation",
    to: a.email,
    subject: `你的預約已確認 · 建達（李建達）`,
    html: transactionalEmailLayout({
      title: "預約已確認",
      preheader: `${esc(a.name)} ${honor}你好,建達已收到你的預約 · ${slotTw}`,
      bodyHtml: custBody,
    }),
  });
}
...
export async function sendCustomerAppointmentConfirmationRequest(a: NotifyInput): Promise<boolean> {
  ...
  const body = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:15px;line-height:1.9;background:#eef8ea;border:1px solid #4faf38;border-radius:10px">
      <tr><td style="padding:14px 16px">
        <div>📅 時段　<b>${esc(slotTw)}</b></div>
        <div style="margin-top:4px">📍 方式　<b>${esc(meetTypeLabel(a.meetType))}</b></div>
        <div style="margin-top:8px;color:#2f6b23">請在台灣時間 ${esc(deadlineTw)} 前完成確認；逾時會自動釋出名額。</div>
      </td></tr>
    </table>
    <div style="text-align:center;margin:22px 0 8px">
      <a href="${esc(manageUrl)}" style="display:inline-block;background:#173d30;color:#ffffff;font-size:15px;font-weight:700;padding:12px 28px;border-radius:8px;text-decoration:none">確認這筆預約</a>
    </div>
    <p style="font-size:13px;color:#64776f;line-height:1.7;margin:14px 0 0">按下確認後，系統才會建立行事曆並寄出正式預約資訊。若不是你本人送出，直接忽略即可。</p>
  `;
  return sendAppointmentEmailAndRecord({
    appointmentId: a.id,
    purpose: "customer_confirmation_request",
    to: a.email,
    subject: "請在 15 分鐘內確認預約 · 建達（李建達）",
    html: transactionalEmailLayout({
      title: "時段已暫時保留",
...
export async function sendCustomerAppointmentChangeEmail(
  ...
  const body = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:15px;line-height:1.9;background:${isCancel ? "#fdf0ef" : "#f7faf8"};border-radius:10px">
      <tr><td style="padding:14px 16px">
        ${change.previousSlotTw ? `<div>原時段　<span style="color:#64776f">${esc(change.previousSlotTw)}</span></div>` : ""}
        <div${change.previousSlotTw ? ` style="margin-top:4px"` : ""}>${isCancel ? "取消時段" : "新時段"}　<b>${esc(slotTw)}</b></div>
        <div style="margin-top:4px">方式　<b>${esc(meetTypeLabel(a.meetType))}</b></div>
        ${!isCancel && a.meetUrl ? `<div style="margin-top:4px">🎥 視訊　<a href="${esc(a.meetUrl)}" style="color:#005335">點我加入 Google Meet</a></div>` : ""}
      </td></tr>
    </table>
    <p style="font-size:14px;color:#64776f;line-height:1.7;margin:18px 0 0">
      ${isCancel ? "如果還需要重新預約,可以回信或直接加建達 LINE。" : "確認信與行事曆請以這封更新後的資訊為準。"}
    </p>
  `;
  return sendAppointmentEmailAndRecord({
    appointmentId: a.id,
    ...
    to: a.email,
    subject: isCancel ? `預約已取消 · 建達（李建達）` : `預約已改期 · 建達（李建達）`,
    html: transactionalEmailLayout({
      title,
...
export async function sendCustomerAppointmentReminder(a: NotifyInput): Promise<boolean> {
  ...
  const slotTw = formatSlotRangeTw(a.slotAt, a.slotEndAt);
  const body = `
    <p style="font-size:16px;line-height:1.7;margin:0 0 16px">${esc(a.name)} ${GENDER_HONOR[a.gender] || ""}你好,提醒你明天有和建達的預約。</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:15px;line-height:1.9;background:#f7faf8;border-radius:10px">
      <tr><td style="padding:14px 16px">
        <div>📅 時段　<b>${esc(slotTw)}</b></div>
        <div style="margin-top:4px">📍 方式　<b>${esc(meetTypeLabel(a.meetType))}</b></div>
    ...
    <p style="font-size:14px;color:#64776f;line-height:1.7;margin:18px 0 0">若需要改期或取消,請直接回信或加建達 LINE。</p>
  `;
  return sendAppointmentEmailAndRecord({
    appointmentId: a.id,
    purpose: "customer_reminder",
    to: a.email,
    subject: `明天預約提醒 · 建達（李建達）`,
    html: transactionalEmailLayout({
      title: "明天預約提醒",
...
export async function notifyNewAppointment(
  ...
  const locEmailHtml = loc
    ? `<br>📍 ${esc(loc.name)}${loc.address ? ` ‧ ${esc(loc.address)}` : ""}` +
      `<br><a href="${esc(mapUrl)}" style="color:#3e8e2c;text-decoration:none">🗺 開地圖導航</a>` +
      (locManualNote ? `<span style="color:#64776f;font-size:13px"> ${locManualNote}</span>` : "")
    : "";
  // 給 LINE 用的地點文字。固定地點（公司面談/分公司/有巢氏房屋工作室）建達本來就知道在哪，
  ...
  const adminBody = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:16px;line-height:1.9;border-collapse:collapse">
      <tr><td style="padding:7px 0;color:#64776f;width:84px;vertical-align:top">姓名</td><td style="padding:7px 0;font-weight:700">${esc(a.name)} ${honor}</td></tr>
      <tr><td style="padding:7px 0;color:#64776f;vertical-align:top">電話</td><td style="padding:7px 0"><a href="tel:${esc(a.phone)}" style="color:#3e8e2c;text-decoration:none">${esc(a.phone)}</a></td></tr>
      <tr><td style="padding:7px 0;color:#64776f;vertical-align:top">Email</td><td style="padding:7px 0">${esc(a.email)}</td></tr>
      ${a.lineId ? `<tr><td style="padding:7px 0;color:#64776f;vertical-align:top">LINE</td><td style="padding:7px 0">${esc(a.lineId)}</td></tr>` : ""}
      <tr><td style="padding:7px 0;color:#64776f;vertical-align:top">需求</td><td style="padding:7px 0;font-weight:700;color:#3e8e2c">${esc(intentStr)}</td></tr>
      <tr><td style="padding:7px 0;color:#64776f;vertical-align:top">急迫度</td><td style="padding:7px 0">${esc(urgencyStr)}</td></tr>
      <tr><td style="padding:7px 0;color:#64776f;vertical-align:top">時段</td><td style="padding:7px 0;font-weight:700">${esc(slotTw)}</td></tr>
      <tr><td style="padding:7px 0;color:#64776f;vertical-align:top">見面</td><td style="padding:7px 0">${esc(meetStr)}${locEmailHtml}${a.meetUrl ? `<br><a href="${esc(a.meetUrl)}" style="color:#3e8e2c;text-decoration:none">🎥 Google Meet 連結</a>` : ""}</td></tr>
    </table>
    ${a.note ? `<div style="margin-top:16px;padding:14px 16px;background:#e9f6e5;border-radius:10px;font-size:16px;line-height:1.7"><b>客戶備註</b><br>${esc(a.note).replace(/\n/g, "<br>")}</div>` : ""}
    ${a.aiSuggestion ? `<div style="margin-top:16px;padding:14px 16px;background:#f2f7f3;border-radius:10px;font-size:15px;line-height:1.7"><b>🤖 AI 研判</b>　${esc(a.aiSuggestion)}</div>` : ""}
    <div style="margin-top:16px;padding:14px 16px;background:#eef8ea;border:1px solid #4faf38;border-radius:12px;font-size:14px;color:#173d30;line-height:1.7">
      <b>內部用</b><br>
      這個入口給建達查看 / 管理這筆預約;不是給客戶看的確認信。
      <div style="text-align:center;margin-top:12px">
        <a href="${esc(internalManageUrl)}" style="display:inline-block;background:#4faf38;color:#173d30;font-size:15px;font-weight:800;padding:11px 18px;border-radius:12px;text-decoration:none">內部用：管理這筆預約</a>
      </div>
    </div>
  `;
  ...
  // ② LINE 給系統擁有者（建達 admin 群,自動 mint token）
  const lineText =
    `🔔 新預約\n` +
    `👤 ${a.name} ${honor}\n` +
    `📞 ${a.phone}\n` +
