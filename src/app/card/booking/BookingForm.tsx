...
const MODE_DESCRIPTIONS: Record<BookingMode, string> = {
  ...
  interview: "應徵有巢氏房屋 頭份中央帝景加盟店相關職務",
};
...
function qualificationFields(mode: BookingMode, intent: string) {
  if (mode === "collaboration") {
    return [
      ...
      { key: "purpose", label: "希望合作的內容", placeholder: "請說明形式、對象與希望建達參與的部分", required: true, multiline: true },
...
export default function BookingForm() {
  ...
  const validate = (): FieldErrors => {
    ...
    if (meetType === "custom" && !customReady) next.meetType = "指定地點必須使用建達核准後提供的專屬連結。";
      ...
      <main className={styles.page}>
        <div className={styles.shell}>
          <header className={styles.header}>
            <div className={styles.brand}>李建達 ‧ 竹南頭份房仲</div>
  ...
  return (
    <main className={styles.page}>
      <div className={styles.shell}>
        <header className={styles.header}>
          <div className={styles.brand}>李建達 ‧ 竹南頭份房仲</div>
          <h1 className={styles.title}>預約與建達聊聊</h1>
          ...
          {bookingMode && intentKey ? (
            <section className={styles.section}>
              ...
              {approvalState === "approved" && !approvedLocation ? (
                <div className={styles.errorNotice}>核准連結缺少已同意的地點，無法開放指定地點。請聯絡建達重新產生連結。</div>
              ) : null}
              {approvalState === "invalid" ? (
                <div className={styles.errorNotice}>指定地點核准連結已失效、已使用或被撤銷，請重新聯絡建達。</div>
              ) : null}
              ...
              {!customReady && approvalState !== "checking" ? (
                <div className={styles.inlineNotice}>
                  需要指定其他地點？請先
                  {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">加建達私人 LINE</a>
                  {" "}確認。取得專屬連結後，系統會直接帶入核准地點。
                </div>
              ) : null}
              {meetType === "custom" && approvedLocation ? (
                <div className={styles.field}>
                  <span className={styles.label}>建達已核准的指定地點</span>
                  ...
                  <p className={styles.sectionHint}>此地點已鎖定，若要更換請先與建達重新確認。</p>
                </div>
              ) : null}
            </section>
          ) : null}
              ...
              <div ref={registerRef("slot")} tabIndex={-1}>
                ...
                {!loadingSlots && !slotError && days.length === 0 ? (
                  <div className={styles.inlineNotice}>
                    ...
                    {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">直接聯絡建達</a>。
                  </div>
                ) : null}
                {days.length ? (
                  <>
